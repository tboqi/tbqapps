...
class Category_Controller extends Admin_Template_Controller {
	...
	function add() {
		$this->initContent('admin/article/category_form', array('title' => '添加分类'));
	}
	...
	function manage() {
		$cate_model = new Category_Model ( );
		$this->initContent('admin/article/category_list', array('categories' => $cate_model->find_all()));
	}
	
	function edit($id) {
		$cate_model = new Category_Model ( );
		$this->initContent('admin/article/category_form', array('category' => $cate_model->get ( $id ), 'id' => $id, 'title' => '编辑分类'));
	}
