...
<table>
    ...
    <tr>
        ...
        <td><select name="category_ids[]" size="6" id="category_ids[]" multiple="multiple">
		  ...
		  if(empty($article) || empty($article->categories)) {
			  ...
			  $cateArr = json_decode($article->categories); 
			  $cateIds = array();
		  ...
		  <option value="<?php echo $category->id; ?>"<?php if (isset($article) && in_array($category->id, $cateIds)) echo ' selected'; ?>><?php echo $category->name; ?></option>
		  <?php } ?>
		</select></td>
    </tr>
    <tr>
    ...
    <tr>
        <td>标签</td>
        <td><input id="tabs" name="tabs" value="<?php echo isset($article) ? article::get_tabs_json($article->tabs) : ''; ?>" type="text" /></td>
    </tr>
    <tr>
        <td>原文地址</td>
