...
<table border="1">
	...
	<tr>
		...
		<td width="18%" rowspan="2">
			<select multiple="multiple" name="category_ids[<?php echo $art->id;?>][]" size="5" id="select">
				...
				<option value="<?php echo $cate->id; ?>"<?php if(article::check_article_has_category($cate->id, $art)) {echo ' selected';} ?>><?php echo $cate->name; ?></option>
				<?php } ?>
			</select>
		</td>
		<td width="18%" rowspan="2">
	...
	<tr>
		<td>标签： <input size="40" type="text" name="tabs[<?php echo $art->id;?>]" id="tabs_<?php echo $art->id;?>" value="<?php echo article::get_tabs_json($art->tabs); ?>" /></td>
	</tr>
	<input type="hidden" name="artIds[]" value="<?php echo $art->id;?>" />
	<?php } ?>
</table>
